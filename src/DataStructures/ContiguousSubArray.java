 ...
 * In the previous post, we checked whether maximum value minus minimum value is equal to ending index minus starting index or not.
 * Since duplicate elements are allowed, we also need to check if the subarray contains duplicate elements or not.
 * For example, the array {12, 14, 12} follows the first property, but numbers in it are not contiguous elements.
 * To check duplicate elements in a subarray, we create a hash set for every subarray and if we find an element already in hash,
